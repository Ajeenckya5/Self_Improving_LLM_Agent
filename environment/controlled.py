"""Controlled environment that executes tool calls within a sandbox."""
...
class ControlledEnvironment:
    ...
    All paths are constrained to env_root.
    """
    ...
    def _resolve(self, path: str) -> Path:
        """Resolve path and ensure it's within env_root."""
    ...
    def _delete_file(self, path: str) -> str:
        ...
        if not p.exists():
            return f"Path does not exist: {path}"
        if p.is_dir():
            return f"Cannot delete directory with delete_file: {path}"
        p.unlink()
    ...
    def _move_file(self, src: str, dst: str) -> str:
        ...
        if sp.is_dir():
            return f"Cannot move directory with move_file: {src}"
    ...
    def _execute_sql(self, sql: str) -> str:
        db = self._db_path()
        if not db.exists():
            return "Database file does not exist yet. Create tables first."
    ...
    def get_state(self) -> dict[str, Any]:
        """Capture current environment state for verification."""
        state = {"env_root": str(self.env_root)}
