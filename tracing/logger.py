...
@dataclass
class TraceStep:
    """Single step in execution: state, action, observation, reasoning."""
    step: int
    state: dict[str, Any]
    action: dict[str, Any]  # e.g. {"tool": "read_file", "args": {"path": "x"}}
    observation: str
    reasoning: str = ""  # agent's thought/reasoning at this step
...
class ExecutionTrace:
    ...
    task_id: str
    agent_type: str  # "baseline" or "strategy_enhanced"
    attempt: int
            ...
            "steps": [
                {
                    ...
                    "reasoning": getattr(s, "reasoning", "") or "",
                }
                for s in self.steps
            ],
        }
...
class TraceLogger:
    """Records and persists execution traces."""
