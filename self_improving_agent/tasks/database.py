...
class CreateTableAndInsertTask(DatabaseTask):
    ...
    def verify(self, env_state: dict[str, Any]) -> TaskResult:
        ...
        try:
            ...
            if not cur.fetchone():
                return TaskResult(success=False, message="Table 'users' does not exist", details={})
            ...
            rows = cur.fetchall()
            expected = [(1, "Alice", "alice@example.com"), (2, "Bob", "bob@example.com")]
            if rows != expected:
                return TaskResult(
                    success=False,
...
class JoinAndAggregateTask(DatabaseTask):
    ...
    def verify(self, env_state: dict[str, Any]) -> TaskResult:
        ...
        try:
            ...
            if not cur.fetchone():
                return TaskResult(success=False, message="View 'customer_totals' not found", details={})
            cur = conn.execute("SELECT name, total FROM customer_totals ORDER BY name")
            rows = sorted(cur.fetchall(), key=lambda r: r[0])
            expected = [("Jane", 75.0), ("John", 150.0)]
